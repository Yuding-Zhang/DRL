...
def make_transition(state, action, reward, next_state, done, log_prob=None):
    transition = {'state': state, 'action': action, 'reward': reward, 'next_state': next_state, 'log_prob': log_prob,
                  'done': done}
    return transition
...
class ReplayBuffer:
    def __init__(self, action_prob_exist, max_size, state_dim, num_action):
        ...
        self.data = {'state': np.zeros((self.max_size, state_dim)), 'action': np.zeros((self.max_size, num_action)),
                     'reward': np.zeros((self.max_size, 1)), 'next_state': np.zeros((self.max_size, state_dim)),
    ...
    def sample(self, shuffle, batch_size=None):
        if shuffle:
            ...
            sampled_data = {'state': self.data['state'][rand_idx], 'action': self.data['action'][rand_idx],
                            'reward': self.data['reward'][rand_idx], 'next_state': self.data['next_state'][rand_idx],
                            'done': self.data['done'][rand_idx]}
            if self.action_prob_exist:
